...
top_acc_T <- show_best(rec_res_T, metric = "accuracy")
best_acc_T <- select_best(rec_res_T, metric = "accuracy")
final_T <- finalize_workflow(rec_wf_T,best_acc_T)

print('**********Tree*********')
tree_test <- last_fit(final_T,income_split)%>%
  collect_metrics()

tree_test %>% print(n = 1)
top_acc_T %>% print(n = 1)

# Combine results into a tibble for later
tree_ans <- top_acc_T %>% slice(1)
...
final_nnet <- finalize_workflow(rec_wf_nnet,best_acc_nnet)
...
final_svm <- finalize_workflow(rec_wf_svm,best_acc_knn)
